...
double mean_value(std::vector<double>& raw_data, double filter)
{
  ...
  int chain_length = raw_data.size();
  //Initialize summation and length of the array
  double sum = 0.0;
  // Adding one by one whole elements of the array
  for(std::vector<double>::iterator i=raw_data.begin()+filter*chain_length;i!=raw_data.end();i++)
  {
    sum += *i;
  }
  // Calculate average of the given sequence
  double result = sum/(chain_length*(1-filter));
  return result;
}
...
{ //Calculate length of the given data
  ...
  return sum/(chain_length*(1-filter));
}
...
{ //Calculate length of the vectors. They are in same size
  int chain_length = x_values.size();
  ...
  double param_b = chain_length*Sum_xy-Sum_x*Sum_y;
  param_b /= chain_length*Sum_xx-Sum_x*Sum_x;
  double param_a = Sum_y-param_b*Sum_x;
  param_a /= chain_length;
  param_a = exp(param_a);
